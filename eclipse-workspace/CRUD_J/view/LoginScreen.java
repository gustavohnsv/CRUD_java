				...
				if (txtUser.getText().equals("") || new String (txtPassword.getPassword()).equals("")) {
					...
					try {
						...
						if (!(txtUser.getText().equals("admin"))) {
							Cryptography cryptoPassword = new Cryptography(new String (txtPassword.getPassword()), Cryptography.MD5); 
							stmt.setString(2, cryptoPassword.toCryptography());// Trecho do código responsável por criptografar senha 
						} else {
							stmt.setString(2, new String (txtPassword.getPassword()));// Se user = admin, não passa pela criptografia
						}
